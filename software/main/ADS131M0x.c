 ...
 * Unmodified from source aside from comments.
 */
...
static void __lock_spi(ads131_t *Ads131){
    if (Ads131->fxn.Lock != NULL) Ads131->fxn.Lock();
}

static void __unlock_spi(ads131_t *Ads131){
    if (Ads131->fxn.Unlock != NULL) Ads131->fxn.Unlock();
}
...
uint32_t _xfer_word(ads131_t *Ads131, uint32_t TxWord){
    ...
    tx_buf[0] = (TxWord >> 16) & 0xFF;
    tx_buf[1] = (TxWord >>  8) & 0xFF;
    tx_buf[2] = (TxWord >>  0) & 0xFF;
    __lock_spi(Ads131);
    __cs_pin(Ads131, _CS_ON);
    __transfer_spi(Ads131, tx_buf, rx_buf, Ads131->WordSize);
    __cs_pin(Ads131, _CS_OFF);
    __unlock_spi(Ads131);
    RxWord = ((uint32_t)rx_buf[0] << 16) |
             ((uint32_t)rx_buf[1] <<  8) |
             ((uint32_t)rx_buf[2] <<  0);
    return RxWord;
}
...
uint32_t _xfer_cmd(ads131_t *Ads131, uint16_t TxCmd){
    ...
    return _xfer_word(Ads131, (uint32_t)TxCmd << 8);
}
...
ads131_err_e _read_regs(ads131_t *Ads131, uint32_t reg, uint16_t *val){
    ...
    _xfer_word(Ads131, (uint32_t)rregCmd << 8);
    status = _xfer_word(Ads131, ADS131_CMD_NULL);

    
    *val = (uint16_t)((status >> 8) & 0xFFFF);
    return ADS131_OK;
}
...
ads131_err_e _read_n_wordsIRQ(ads131_t *Ads131, uint32_t Words){
    ...
    __lock_spi(Ads131);
    __cs_pin(Ads131, _CS_ON);
    err = __transfer_spi_irq(Ads131, Ads131->_intern.BufferForTxIrq,
                             Ads131->_intern.BufferForRxIrq, nBytes);
    return err;
}
...
ads131_err_e ads131_write_reg(ads131_t *Ads131, uint32_t reg, uint16_t val){
    if (Ads131 == NULL) return ADS131_UNKNOWN;
    if (Ads131->_intern.Initialized != ADS131M0_DRIVER_INITILIZED) return ADS131_NOT_INITIALIZED;
...
ads131_err_e ads131_set_osr(ads131_t *Ads131, ads131_osr_value_e Osr){
    ...
    if (Ads131 == NULL) return ADS131_UNKNOWN;
    if (Ads131->_intern.Initialized != ADS131M0_DRIVER_INITILIZED) return ADS131_NOT_INITIALIZED;
    if (Osr == ADS131_OSR_64 && Ads131->nChannels > 4) return ADS131_INVALID_PARAMS;
    _read_regs(Ads131, ADS131_REG_CLOCK, &Clock._raw);
    if (Osr == ADS131_OSR_64) { Clock.TBM = 1; }
    else { Clock.TBM = 0; Clock.OSR = (uint8_t)Osr; }
    _write_regs(Ads131, ADS131_REG_CLOCK, Clock._raw);
    _update_samplerate(Ads131);
    return ADS131_OK;
}
...
ads131_err_e ads131_read_all_channel(ads131_t *Ads131, ads131_channels_val_t *val){
    uint32_t Response[ADS131_MAX_CHANNELS_CHIPSET + 2];  
    uint8_t i, offset;
    if (Ads131 == NULL) return ADS131_UNKNOWN;
    if (Ads131->_intern.Initialized != ADS131M0_DRIVER_INITILIZED) return ADS131_NOT_INITIALIZED;
    
    uint32_t words_to_read = 1 + Ads131->nChannels + 1;
    _read_n_words(Ads131, Response, words_to_read);
    offset = 1;  
    for (i = 0; i < Ads131->nChannels; i++){
        uint32_t raw = Response[offset++];
        
        if (raw & 0x800000){
            raw |= 0xFF000000;  
        }
        val->ChannelRaw[i] = (int32_t)raw;
...
ads131_err_e ads131_read_one_channel(ads131_t *Ads131, ads131_channel_e Channel, uint32_t *raw, ads131Float_t *miliVolt){
    ...
    if (Ads131 == NULL) return ADS131_UNKNOWN;
    if (Ads131->_intern.Initialized != ADS131M0_DRIVER_INITILIZED) return ADS131_NOT_INITIALIZED;
    if (Channel > Ads131->nChannels) return ADS131_CHANNEL_DOENST_EXISTS;
    err = ads131_read_all_channel(Ads131, &val);
    if (err) return err;
    if (raw != NULL)      *raw      = val.ChannelRaw[Channel];
    if (miliVolt != NULL) *miliVolt = val.ChannelVoltageMv[Channel];
    return ADS131_OK;
}
...
ads131_channels_val_t ads131_spi_transfer_irq(ads131_t *Ads131){
    ...
    if (Ads131 == NULL) return *ChannelVal;
    __cs_pin(Ads131, _CS_OFF);
    __unlock_spi(Ads131);
    offset = 0;
    for (i = 0; i < Words; i++){
